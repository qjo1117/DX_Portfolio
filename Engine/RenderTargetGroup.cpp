...
void RenderTargetGroup::Create(RENDER_TARGET_GROUP_TYPE groupType, vector<RenderTarget>& rtVec, Ref<Texture> dsTexture)
{
	/* ----- 맵핑을 해준다. ----- */
	...
	/* ----- 모든 버퍼를 담을 DescriptorHeap을 만들어준다. ----- */
...
{
	D3D12_VIEWPORT vp = D3D12_VIEWPORT{ 0.f, 0.f, static_cast<float>(_rtVec[0].target->GetWidth()) , static_cast<float>(_rtVec[0].target->GetHeight()), 0.f, 1.f};
