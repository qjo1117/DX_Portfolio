...
void FBXLoader::Import(const wstring& path)
{
	...
	string strPath = Utils::Wstr2Str(path);
...
void FBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	meshInfo.name = Utils::Str2Wstr(mesh->GetName());
...
void FBXLoader::LoadMaterial(FbxSurfaceMaterial* surfaceMaterial)
{
	...
	material.name = Utils::Str2Wstr(surfaceMaterial->GetName());
	...
	for (size_t i = 0; i < _meshes.size(); i++)
	{
		for (size_t j = 0; j < _meshes[i].materials.size(); j++)
		{
...

			{
				...
				Ref<Texture> diffuseTexture = GET_SINGLE(Resources)->Get<Texture>(key);
				if (diffuseTexture)
...

			{
				...
				wstring key = filename;
				Ref<Texture> normalTexture = GET_SINGLE(Resources)->Get<Texture>(key);
				if (normalTexture)
...

			{
				...
				wstring key = filename;
				Ref<Texture> specularTexture = GET_SINGLE(Resources)->Get<Texture>(key);
				if (specularTexture)
...
void FBXLoader::LoadBones(FbxNode* node, int32 idx, int32 parentIdx)
{
	...
	{
		Ref<FbxBoneInfo> bone = make_shared<FbxBoneInfo>();
		bone->boneName = Utils::Str2Wstr(node->GetName());
...
void FBXLoader::LoadAnimationInfo()
{
	...
	for (int32 i = 0; i < animCount; i++)
	{
		...
		Ref<FbxAnimClipInfo> animClip = make_shared<FbxAnimClipInfo>();
		animClip->name = Utils::Str2Wstr(animStack->GetName());
...
void FBXLoader::LoadKeyframe(int32 animIndex, FbxNode* node, FbxCluster* cluster, const FbxAMatrix& matNodeTransform, int32 boneIdx, FbxMeshInfo* meshInfo)
{
	...
	// TODO : Animation Matrix 내가 건들어야할 내용
