...
void Shader::CreateGraphicsShader(const wstring& path, ShaderInfo info, ShaderArg arg)
{
	...
	wstring basePath = SHADER_PATH + path;

	CreateVertexShader(basePath, arg.vs, "vs_5_0");

	if (arg.hs.empty() == false) {
		CreateHullShader(basePath, arg.hs, "hs_5_0");
	}
	if (arg.ds.empty() == false) {
		CreateDomainShader(basePath, arg.ds, "ds_5_0");
	}
	if (arg.gs.empty() == false) {
		CreateGeometryShader(basePath, arg.gs, "gs_5_0");
	}

	CreatePixelShader(basePath, arg.ps, "ps_5_0");
