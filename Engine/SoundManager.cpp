...
bool SoundManager::Init(void)
{
	...
	vector<Ref<FileInfo>> infos;
	GET_SINGLE(DirectoryManager)->FindFileInfo(GET_SINGLE(DirectoryManager)->GetFileInfo(), infos, ".mp3");

	for (Ref<FileInfo> info : infos) {
		LoadSound(info->Name, false, info->Name.c_str());
	}
